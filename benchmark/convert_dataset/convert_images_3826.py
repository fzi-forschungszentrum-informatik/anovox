...
def move_to_cityscapes(anovox_root, cityscapes_raw_dir, cityscapes_gt_dir):
    ...
    for scenario in tqdm(sorted(os.listdir(anovox_root))):
        ...
        # if file_count > 695: # only so much needed for first evaluation (100 * 5)
        #     break
        # if file_count > 744: # for 4 images per scenario
        # if file_count > 3:
        # if file_count > 595: # 595 * 5 = 2975
...
def create_cityscapes_folders(root):
    ...
    cityscapes_dir = os.path.join(root_prev, "cityscapes_19130")
    ...
    for scenario_dir in sorted(os.listdir(root)):
        ...
        # if len(os.listdir(raw_data_train_dir)) >= 595: # create val folders for 5
