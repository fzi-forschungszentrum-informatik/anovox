...
from Models.World.local_planner import (
    LocalPlanner,
    RoadOption,
)
from Models.World.misc import (
    get_speed,
    is_within_distance,
...
class BasicAgent(object):
    ...
    def __init__(
        ...
        if map_inst:
            if isinstance(map_inst, carla.Map):
                ...
                logging.warning("Ignoring the given map as it is not a 'carla.Map'")
    ...
    def run_step(self):
        ...
        (
            ...
            _,
            _,
        ) = self._vehicle_obstacle_detected(vehicle_list, max_vehicle_distance)
        ...
        (
            affected_by_tlight,
            _,
        ) = self._affected_by_traffic_light(self._lights_list, max_tlight_distance)
    ...
    def lane_change(
        ...
        if not path:
            logging.warning("Ignoring the lane change as no path was found")
    ...
    def _vehicle_obstacle_detected(
        ...
        def get_route_polygon():
            ...
            for (
                    wp,
                    _,
            ) in self._local_planner.get_plan():
