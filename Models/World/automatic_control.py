...
from constant_velocity_agent import (
...
def find_weather_presets():
    ...
    return [
        (
            getattr(carla.WeatherParameters, x),
            name(x),
        )
        for x in presets
    ]
...
class World(object):
    ...
    def __init__(self, carla_world, hud, args):
        ...
        try:
            ...
            logging.error(f"RuntimeError: {error}")
            logging.error("The server could not send the OpenDRIVE (.xodr) file:")
            logging.error("Make sure it exists, has the same name as your town, and is correct.")
            sys.exit(1)
        self.hud = hud
        self.player = None
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        def dist(l):
            return math.sqrt((l.x - transform.location.x) ** 2 + (l.y - transform.location.y) ** 2 + (
